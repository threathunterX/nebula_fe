...
@withRouter
class UserClickAnalysis extends Component {
  static propTypes = {
  ...
  onSearchSubmit(keyword) {
    this.props.history.push(`/analysis/user/clickStream/${encodeURIComponent(keyword)}`);
  ...
  fetchRelatedStatistics() {
    ...
    FetchSlotData({
      ...
      onSuccess: (result) => {
        ...
        // 格式化点击数
        const clicks =
          _.map(value, item => ({ x: item.time_frame, y: _.get(item[slotMap.USER_DYNAMIC_COUNT], 'value') }));
  ...
  render() {
    ...
    return (
      <div className="wd-user-analyze container">
        ...
        <RedqInfo keyType="mobile" keyValue={decodeURIComponent(params.key)} />
        ...
        >
          <div
            ...
            onClick={() => {
              history.push(`/analysis/user/${encodeURIComponent(params.key)}?timestamp=${timestamp}`);
            }}
            role="presentation"
          >
...
export default UserClickAnalysis;
